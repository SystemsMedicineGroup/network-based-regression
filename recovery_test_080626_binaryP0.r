...
m  <- 500
P0 <- matrix(rbinom(m * n, 1, 0.3), m, n) # 30% mutated (binary)
...
print(w_true) # 4
print(RWR_normal$w) # -1.67979

print(v_true) # 0 5 0
print(RWR_normal$v) # 1.26 4.999 -4.113165e-06
...
cor(y_true, as.numeric(RWR_normal$P %*% RWR_normal$v)) #1
...
plot(y_pred, y_true, xlab = "Predicted Growth Rate", ylab = "Observed (true) Growth Rate",
     xlim = range(c(y_true, y_pred)), ylim = range(c(y_true, y_pred)), main = "Goodness of Fit (y)", 
     pch = 16, cex = 1, col = rgb(0, 0, 1, 0.4))
...
# The only change is 0ing value of v1 in lines 134, 144, 161, 167 
...
print(w_true) # 4
print(RWR_fix_v1$w) # 3.114002

# better 

print(v_true) # 0 5 0
print(RWR_fix_v1$v) # 0, 5.01, 0.004

# also better than before. Signal does not go towards gene 1 (of course), but 
# goes to gene 3 even less 

print(cor(y_true, as.numeric(RWR_fix_v1$P %*% RWR_fix_v1$v))) # 0.9999953
...
plot(y_pred1, y_true, xlab = "Predicted Growth Rate", ylab = "Observed (true) Growth Rate",
     xlim = range(c(y_true, y_pred)), ylim = range(c(y_true, y_pred)), main = "Goodness of Fit (Fixed v1)", 
     pch = 16, cex = 1, col = rgb(0, 0, 1, 0.4))
abline(0, 1, col = "red", lwd = 2)
