...
#sparse n × n matrix Q where Q[i, j] = a_ij / r[i]

eq2_transition <- function(A, net) {
  r <- rowSums(A) #total activation from each row (gene)                                                              
  Q <- Diagonal(x = 1 /(r + 1e-8)) %*% A  #1/r[i] (1/total activation of each gene) on the diagonal of a matrix. Then divides by A                                                    
  return(Q)
}

#### EQUATION 3 RWR 
#returns m × n dense matrix.
...
eq3_rwr <- function(P0, Q, alpha = 0.5, rtol = 1e-5, atol = 1e-8, max_iter = 100) { 
  ...
  for (t in seq_len(max_iter)) {   #iteration loop
    ...
    if (max(abs(P_new - P) - rtol * abs(P)) <= atol) {  #convergence check, each entry of P tol relative to its scale
...
eq4_objective <- function(w, v, P, y, lambda) {
  ...
  J <- lambda * sum(abs(w)) + lambda * sum(abs(v)) + (1 / (2 * m)) * sum(resid^2)   #the equation 
  return(J)
}

#### EQUATION 5, OBJECTIVE IN SUMMATION FORM 
#won't be used, just cross check against eq4
#would take much longer (nested loops)

eq5_objective_sum <- function(w, v, P, y, lambda) {
  m <- length(y) #number of tumours
  n <- length(v) #number of genes
  total <- 0  #initialise running sum for squared error terms
  for (c in seq_len(m)) {  #outer loop for tumours 
    pred <- 0 #sets initial prediction of tumour c to 0
    for (i in seq_len(n)) { #inner loop for genes
      pred <- pred + P[c, i] * v[i] #calculates pred, accumulates. Adds new gene (i's) contribution
    }
    total <- total + (y[c] - pred)^2 #takes squared residual for this tumour, adds that to total
  }
  J_summation <-lambda * sum(abs(w)) + lambda * sum(abs(v)) + (1 / (2 * m)) * total #the equation w final total
  return(J_summation) #should return same value as eq4. (checked, it does)
}                                
...
eq6_dA_dw <- function(a_edges, net) {                                   
  ...
  dA_dw <- net$X * logistic_slope #E × N, each row of net$X*logstic_slope row (corresponding). The equation.
  return(dA_dw)
}
...
eq10_dJ_dwl <- function(w_l, dP_dwl, v, y, P, lambda) {                         
  ...
  dJ_dwl <- sign(w_l) * lambda - (1 / m) * as.numeric(t(dP_dwl %*% v) %*% resid) #the equation
  return(dJ_dwl)
}

#### GRADIENT COMP. FOR ALL FEATURES, PARALLEL. Eq7.9.10

compute_dJ_dw <- function(w, v, A, Q, P, y, net, dA_dw, alpha, lambda, rtol, atol, max_iter, n_cores = 1) { #ncores can be changed if testing only this function
  solver <- function(l) {   
    ...
    dJ_dwl <- eq10_dJ_dwl(w[l], dP_dwl, v, y, P, lambda) #eq10
    return(dJ_dwl)
  }
...
#### EQUATION 12 
#not used because we are using Adam instead
#eq12_update_w <- function(w, dJ_dw, eta) {
#  w - eta * dJ_dw
#}


#### EQUATION 13 dJ/dv
#length n numeric vector, entry i is ∂J/∂v_i, goes straight into Adam update for v

eq13_dJ_dv <- function(v, P, y, lambda) {
  m <- length(y) #number of tumours
  resid <- y - as.numeric(P %*% v) #same as in eq4 and eq10
  dJ_dv <- sign(v) * lambda - (1 / m) * as.numeric(t(P) %*% resid) #the equation
  return(dJ_dv)
}
...
#### EQUATION 14
#not used because we are using adam instead
#eq14_update_v <- function(v, dJ_dv, eta) {
#  v - eta * dJ_dv
#}
...
eq15_dJ_dvi <- function(i, v, P, y, lambda) {
  ...
  dJ_dvi <- sign(v[i]) * lambda - (1 / m) * as.numeric(t(P[, i]) %*% resid) #the equation. Same concept, just for single genes
  return(dJ_dvi)
}
...
init_adam <- function(size) { #the size of whats being optimised, w (N) and v (n)
  state <- list(m = rep(0, size), v = rep(0, size), t = 0L) #m: first moment estimate, v:second moment estimate. both set to 0. T is timestwp counter, counting updates. 
  return(state)
}

### Adam update step, instead of eq12, 14

#theta: parameter vector being updated (w or v), grad: eq 10, state: from init_adam
...
rwr_lasso_train <- function(P0, edges, X, y, alpha = 0.5, lambda = 0.01, eta = 0.001, beta1 = 0.9, beta2 = 0.999,
                            eps = 1e-8, max_outer = 500, conv_threshold_outer = 1e-5, rtol = 1e-5, atol = 1e-8,
                            max_iter = 100, n_cores = max(1, detectCores() - 1)) { #for parrallel! 
  ...
  J_history <- numeric(max_outer) #J_history set to max outer iterations 500, for tracking
  ...
  for (iter in seq_len(max_outer)) {                                            
    ...
    J_history[iter] <- eq4_objective(w, v, P, y, lambda) #record current J and store it for this iter
    
    dA_dw <- eq6_dA_dw(a_edges, net) #eq6 using a_edges, E x N grad. of activation for this iteration
    
    #parrallel computation of eq7, 9, 10
    dJ_dw <- compute_dJ_dw(w, v, A, Q, P, y, net, dA_dw, alpha, lambda, rtol, atol, max_iter, n_cores = n_cores)
    dJ_dv <- eq13_dJ_dv(v, P, y, lambda)  #eq13                                     
  ...
  result <- list( #if not converged, continue until max_outer (500)
    w = w,
    v = v,
    A = A,
    Q = Q,
